...
module open_taper_shell(lid_l, lid_w, lid_t, wallt) {
    ...
    // --- Front wall (tapered top + vertical bottom)
    difference() {
        union() {
            // Tapered 3/4 section
            hull() {
...
translate([0, lid_w/2 + 1.98, 0.4])      // position slightly forward and up
  ...
  //------------------------------------------------
// Side walls following taper and vertical front
//------------------------------------------------
module side_wall(xsign = 1) {
    wall_thick = wallt;

    // convenient local Y positions
    back_y   = -lid_w/2 + wallt/2;
    front_y  =  lid_w/2 + front_vertical_out;
    top_z    =  lid_t/2;
    bot_z    =  bottom_z;

    // build outer & inner contours as 4-point polylines
    outer = [
    ...
    // build faces by stitching between outer and inner profiles
    polyhedron(
